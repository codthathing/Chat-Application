...
def group_user_choices(user: User, userDetails: Callable[[User], None], choice, group_users: list[User] | None) -> list[User] | None:
    match choice:
        case 1:
...
def create_group_options(user: User, userDetails: Callable[[User], None], choice, group_users: list[User] | None = None) -> list[User] | None:
...
def enter_chat(userDetails: Callable[[User], None], user: User, room: MultiChatRoom, group_name: str):
    message: str = input("\nNew message: ")
...
def add_group_member_action(userDetails: Callable[[User], None], user: User, room: MultiChatRoom, group_name: str, prompt: str, action: Callable[[User, User], str], not_found_message: Callable[[str], str]) -> None:
    new_username: str = input(prompt)

    target_user: User | None = next((u for u in User.users if u.username == new_username), None)
    ...
    if target_user:
        ...
        print(not_found_message(new_username))

    group_chat_details(userDetails, user, room, group_name)

def add_user(userDetails: Callable[[User], None], user: User, room: MultiChatRoom, group_name: str):
    add_group_member_action(userDetails, user, room, group_name, "\nEnter new username: ", room.addUserToRoom, lambda name: f"\n@{name} not a user, can't add to group!",)

def add_admin(userDetails: Callable[[User], None], user: User, room: MultiChatRoom, group_name: str):
    add_group_member_action(userDetails, user, room, group_name, "\nEnter new admin username: ", room.addRoomAdmin, lambda name: f"\n@{name} not a user!",)

def user_view_group_options(choice: int, userDetails: Callable[[User], None], user: User, room: MultiChatRoom, group_name: str) -> None:
    match choice:
        case 1:
        ...
        case _:
            ...
            user_view_group_options(choice, userDetails, user, room)

def user_view_group_admin_options(choice, userDetails: Callable[[User], None], user: User, room: MultiChatRoom, group_name: str) -> None:
    match choice:
        case 1:
            enter_chat(userDetails, user, room, group_name)
        case 2:
        ...
        case _:
            ...
            user_view_group_admin_options(choice, userDetails, user, room)

def determine_user_group_options(userDetails: Callable[[User], None], user: User, room: MultiChatRoom, group_name: str) -> None:
...
def group_chat_details(userDetails: Callable[[User], None], user: User, room: MultiChatRoom, group_name: str):
...
def group_created_options(choice, userDetails: Callable[[User], None], user: User, room: MultiChatRoom, group_name: str) -> None:
    match choice:
        case 1:
            group_chat_details(userDetails, user, room, group_name)
        case 3:
...
def create_group(user: User, userDetails: Callable[[User], None]) -> None:
    group_name = input("\nEnter a group name: ")
    ...
    if group_users is not None:
        ...
        user.addGroup(group_name, multi_room)
